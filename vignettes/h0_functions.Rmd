...
author: "Thomas Matheis, Phineus Choi, Sam Butler, Mira Terdiman, Jo Hardin"
date: "`r Sys.Date()`"
...
```{r generate_data}
time <- rep(seq(1, 24, 0.5), 5)
noise_parameter <- 1.5
mean_vals <- doublesigmoidalFitFormula_h0(time,
...
intensity <- rnorm(n = length(mean_vals), mean = mean_vals, sd = rep(noise_parameter, length(mean_vals)))

dataInput <- data.frame(time, intensity)
ggplot(dataInput, aes(time, intensity)) + 
  geom_point() + 
  scale_y_continuous(limits = c(-4, 14), expand = expansion(mult = c(0, 0))) + 
  theme_bw()
```
